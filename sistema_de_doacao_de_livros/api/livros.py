...
async def criar_livro(
    ...
    if foto and foto.filename:
        ...
        foto_url = f"/tmp/sistema-de-doacao-de-livros/fotos_livros/{nome_arquivo}"
...
@rotas_api_livros.get("/livros", response_model=list[LivroComStatus])
def listar_livros():
    return [
        LivroComStatus(
            id=livro.id,
            ...
            em_doacao_pendente=banco_de_dados.livro_esta_em_doacao_pendente(livro.id),
        )
...
@rotas_api_livros.put("/livros/{livro_id}", response_model=Livro)
async def atualizar_livro(
    ...
    if banco_de_dados.livro_esta_em_doacao_pendente(livro_id):
        raise HTTPException(
            status_code=HTTPStatus.BAD_REQUEST,
            detail="Não é possível editar este livro pois ele está em uma doação pendente",
        )
...
@rotas_api_livros.delete("/livros/{livro_id}", status_code=HTTPStatus.NO_CONTENT)
def excluir_livro(livro_id: int):
    ...
    if banco_de_dados.livro_esta_em_doacao_pendente(livro_id):
        raise HTTPException(
            status_code=HTTPStatus.BAD_REQUEST,
            detail="Não é possível excluir este livro pois ele está em uma doação pendente",
        )
